#include "../quadprog_eigen/EigenQP.h"
#include "QuadProg.h"
#define QUADPROG_EIGEN
...
double SolveQP(const MatrixXd& gm, const VectorXd& gv,
               ...
               VectorXd& xv)
{
#ifdef QUADPROG_EIGEN
    MatrixXd gm_copy(gm);
    // Copy the matrix and condition it due to being underconstrained.
    gm_copy += MatrixXd::Identity(gm_copy.rows(), gm_copy.cols()) * 1e-05;
    VectorXd gv_copy(gv);
    double retval = QP::solve_quadprog(gm_copy, gv_copy, cem.transpose(), cev, cim.transpose(), civ, xv);
#else
