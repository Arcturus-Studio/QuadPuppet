...
//Preconditions: Matrix must be square and symmetric.
bool checkPositiveDefinite(Matrix<double>& G)
{
	int nrows = G.nrows();
	for (int elimRow = 0; elimRow < nrows; elimRow++)
	{	
		//Find a non-eliminated row with elimRow'th column non-zero
		int swapRow = -1;
		for (int i = elimRow; i < nrows; i++)
		{
			if (fabs(G[i][elimRow]) > 1e-9)
			{
				swapRow = i;
				break;
			}
		...
		//Swap row into place (if it's not already in place)
		if (swapRow != elimRow)
		{
			Vector<double> temp = G.extractRow(swapRow);
			G.setRow(swapRow, G.extractRow(elimRow));
			G.setRow(elimRow, temp);
		}
		//Eliminate all later row's elim-th column
		for (int i = elimRow + 1; i < nrows; i++)
		{
			Vector<double> temp = G.extractRow(i);
			temp = temp - G.extractRow(elimRow) * (temp[elimRow] / G[elimRow][elimRow]);
			temp[elimRow] = 0; //Exactly zero out the eliminated column, mostly for pretty-print
			G.setRow(i, temp);
		}
	}
	//print_matrix("Eliminated", G);
	//Check all diagonal entries are now positive
