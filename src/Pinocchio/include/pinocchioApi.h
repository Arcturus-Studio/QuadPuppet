...
PinocchioOutput autorig(const Skeleton &given, const Pinocchio::Mesh &m);

//============================================individual steps=====================================

//fits mesh inside unit cube, makes sure there's exactly one connected component
Pinocchio::Mesh prepareMesh(const Pinocchio::Mesh &m);
...
TreeType *constructDistanceField(const Pinocchio::Mesh &m, double tol = defaultTreeTol);
...
bool SaveModelFBX(const char* savePath, std::vector<Vector3> localEmbedding, std::vector<Vector3> localEulers, Attachment* attachment, Pinocchio::Mesh mesh, Skeleton* skel, std::vector<int> closestIndex);
bool SaveJustAMesh( Pinocchio::Mesh mesh, int i);
