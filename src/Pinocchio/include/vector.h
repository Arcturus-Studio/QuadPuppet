...
public:
    ...
    Vector(const Real &m1, const Real &m2, const Real &m3, const Real &m4) { m[0] = m1; m[1] = m2; m[2] = m3; m[3] = m4; checkDim<4>(VO()); }
...
//operators
    ...
    Self operator*(const Real& scalar) const { 
        std::function<Real(Real)> f = [scalar](Real x) -> Real { return x * scalar; };
        return apply(f); 
    }
    Self operator/(const Real &scalar) const {
        std::function<Real(Real)> f = [scalar](Real x) -> Real { return x / scalar; };
...
template <class Real, int Dim>
template <typename Writer>
void Vector<Real, Dim>::Serialize(Writer&  writer)
{
...
template <class Real, int Dim>
template <typename Document>
void Vector<Real, Dim>::Deserialize(Document& doc)
{
	m[0] = doc["x"].GetDouble();
	if (Dim >= 2)
		m[1] = doc["y"].GetDouble();
