...
public:
	...
	const std::vector<std::string> &names() const { return fNameV; }
...
public:
    HumanSkeleton();
    HumanSkeleton(std::map<std::string, Vector3> embeddings);

    static const int jointCount;
	static const char* jointOrder[];
};
