...
#define MIN_COMPONENT_SIZE_FOR_HEAD 5
...
public:
    ...
    Attachment(const Pinocchio::Mesh& mesh, const std::vector<int>& parents, const std::vector<Vector3>& match, const VisibilityTester* tester, const QuadPuppet::RiggingParameters::RiggingSource rigType, const QuadPuppet::RiggingParameters& params);
    virtual ~Attachment();

    Pinocchio::Mesh deform(const Pinocchio::Mesh& mesh, const std::vector<Transform<>>& transforms) const;
    Vector<double, -1> getWeights(int i) const;
    size_t getWeightsSize() const;
    std::vector<QuadPuppet::BoneWeight> getBoneWeights();
