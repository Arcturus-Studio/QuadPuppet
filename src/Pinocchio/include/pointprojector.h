...
public:
    ...
    Vec project(const Vec &from) const
    {
        ...
        // We need an array of 10000 items to use per thread. 
        // This is kind of gross but cuts down on allocations significantly.
        static map<size_t, std::array<pair<double, int>, 10000>> todo;
        size_t threadID = std::hash<std::thread::id>{}(std::this_thread::get_id());
        if (todo.find(threadID) == todo.end())
        {
            todo[threadID] = std::array<pair<double, int>, 10000>();
        }

        int sz = 1;
        todo[threadID][0] = make_pair(rnodes[0].rect.distSqTo(from), 0);

        while(sz > 0) 
        {
            if (todo[threadID][--sz].first > minDistSq)
            {
                continue;
            }

            int cur = todo[threadID][sz].second;      // the top element that was just popped
            ...
            { 
                ...
                if (l1 < minDistSq)
                {
                    todo[threadID][sz++] = make_pair(l1, c1);
                }
            
                double l2 = rnodes[c2].rect.distSqTo(from);
                if (l2 < minDistSq)
                {
                    todo[threadID][sz++] = make_pair(l2, c2);
                }
            
                
                if (sz >= 2 && todo[threadID][sz - 1].first > todo[threadID][sz - 2].first)
                {
                    swap(todo[threadID][sz - 1], todo[threadID][sz - 2]);
                }
