...
HumanSkeleton::HumanSkeleton()
{
	...
	// TODO: look into why 0.7.
	scale(0.7);
}

HumanSkeleton::HumanSkeleton(std::map< std::string, Vector3> embeddings)
{
	makeJoint("hips",		embeddings["hips"]);
	makeJoint("back",		embeddings["back"],			"hips");			//1
	makeJoint("shoulders",	embeddings["shoulders"],	"back");            //2
	makeJoint("head",		embeddings["head"],			"shoulders");       //3

	makeJoint("lthigh",		embeddings["lthigh"],		"hips");            //4
	makeJoint("lknee",		embeddings["lknee"],		"lthigh");          //5
	makeJoint("lankle",		embeddings["lankle"],		"lknee");           //6
	makeJoint("lfoot",		embeddings["lfoot"],		"lankle");          //7

	makeJoint("rthigh",		embeddings["rthigh"],		"hips");            //8
	makeJoint("rknee",		embeddings["rknee"],		"rthigh");          //9
	makeJoint("rankle",		embeddings["rankle"],		"rknee");           //10
	makeJoint("rfoot",		embeddings["rfoot"],		"rankle");          //11

	makeJoint("lshoulder",	embeddings["lshoulder"],	"shoulders");       //12
	makeJoint("lelbow",		embeddings["lelbow"],		"lshoulder");       //13
	makeJoint("lhand",		embeddings["lhand"],		"lelbow");          //14

	makeJoint("rshoulder",	embeddings["rshoulder"],	"shoulders");       //15
	makeJoint("relbow",		embeddings["relbow"],		"rshoulder");       //16
	makeJoint("rhand",		embeddings["rhand"],		"relbow");          //17
...
const int HumanSkeleton::jointCount     = 18;
const char* HumanSkeleton::jointOrder[] = { "hips", 
	                                        "back",
	                                        "shoulders",
	                                        "head",
	                                        "lthigh",
