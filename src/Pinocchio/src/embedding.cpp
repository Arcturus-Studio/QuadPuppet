...
vector<Vector3> splitPath(FP *fp, int joint, int curIdx, int prevIdx)
{
    ...
    uncompIdx.reserve((int)fp->given.fcMap().size());
    ...
    if(newPath.size() > 1) { //if there is a meaningful path in the extracted graph
        ...
        lengths.reserve((int)uncompIdx.size());
...
//distance penalty
// Return range seems to be 0. - 3. but could actually return values higher then that. 
...
public:
    ...
    double get(const PartialMatch &cur, int next, int idx) const
    {
        ...
        double optDist = fp->given.cLength()[idx]; // length of the bone in the compressed graph
...
double maxdist = 0.1;
//temporal cohesion penalty
class MotionPF : public PenaltyFunction
{
public:
    MotionPF(FP *inFp) : PenaltyFunction(inFp) { }
    double get(const PartialMatch &cur, int next, int idx) const
    {
		double out = 0;
		Skeleton* pSkel = fp->given.prevSkeleton;
		if (pSkel == NULL) return 0;

		int count = pSkel->cGraph().verts.size();

		double dist = 30. * SQR((pSkel->cGraph().verts[idx] - fp->given.cGraph().verts[idx]).lengthsq());
		// Try to preserve bone length?
		//double dist = 3. * SQR(pSkel->cLength()[idx] - fp->given.cLength()[idx]);
		out += dist;
        return out;
    }
};
...
vector<PenaltyFunction *> getPenaltyFunctions(FP *fp) //user responsible for deletion of penalties
{
	...
	out.push_back(new MotionPF(fp));

    double weights[10] = { 0.027, 0.023, 0.007, 0.046, 0.014, 0.012, 0.072, 0.005, 0.033, 0.05 };
