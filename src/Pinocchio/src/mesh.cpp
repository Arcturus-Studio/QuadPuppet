...
void Mesh::readObj(istream &strm)
{
	...
	while (!strm.eof() && !cin.fail())
	{
		...
		if (words[0] == "f") {
			...
			int a[16][3]; // [vert, uv, normal]
			for (i = 0; i < (int)words.size() - 1; ++i)
			{
				// Format: vert/uv/normal
				if (words[i + 1].find("/") != std::string::npos)
				{
					sscanf(words[i + 1].c_str(), "%d/%d/%d", &a[i][0], &a[i][1], &a[i][2]);
					vertsOnly = false;
				}

				// Format: vert
				else
					sscanf(words[i + 1].c_str(), "%d", &a[i][0]);
			}
			...
			for (int j = 2; j < (int)words.size() - 1; ++j)
			{
				...
				edges[first].vertex = a[0][0] - 1;
				edges[first + 1].vertex = a[j - 1][0] - 1;
				edges[first + 2].vertex = a[j][0] - 1;
			}

			faces.push_back(a[0][0] - 1);
			faces.push_back(a[1][0] - 1);
			faces.push_back(a[2][0] - 1);

			if (!vertsOnly)
			{
				faces_uvs.push_back(a[0][1] - 1);
				faces_uvs.push_back(a[1][1] - 1);
				faces_uvs.push_back(a[2][1] - 1);
			}
		}
	...
	//std::cout << "temp_uvs  " << tempUVs.size() << std::endl;
	//std::cout << "faces  " << faces.size() << std::endl;
	//std::cout << "face uvs " << faces_uvs.size() << std::endl;
}
...
void Mesh::writeObj(const string &filename) const
{
	...
	for (i = 0; i < (int)uvs.size(); ++i)
		os << "vt " << uvs[i][0] << " " << uvs[i][1] << endl;

	if (faces_uvs.size() == faces.size())
	{
		for (i = 0; i < (int)faces.size(); i += 3)
			os << "f " << faces[i] + 1 << "/" << faces_uvs[i] + 1
			<< " " << faces[i + 1] + 1 << "/" << faces_uvs[i + 1] + 1
			<< " " << faces[i + 2] + 1 << "/" << faces_uvs[i + 2] + 1 << endl;
	}
	else {
		for (i = 0; i < (int)edges.size(); i += 3)
...
#define CHECK(pred) { if(!(pred)) { std::cout << "Mesh integrity error: " #pred << " in line " << __LINE__ << endl; return false; } }
